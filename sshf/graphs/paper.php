...
if ($stmt = $mysqli->prepare("SELECT paper_avrg, paper_wish 
                                      FROM graphs LIMIT 1")){
            $stmt->execute();  
            $stmt->store_result();
...
$graph->xaxis->SetTickLabels(array('Per dag','Per uke','Per måned','Per år'));
 /*
$graph->xaxis->title->Set('Tenk gjennomsnitlig antall ark * ansatte!');
$graph->xaxis->title->SetFont(FF_FONT1,FS_BOLD);
*/
$graph->yaxis->title->Set('Antall ark');
$graph->yaxis->title->SetFont(FF_FONT1,FS_BOLD);
$graph->yaxis->title->SetMargin(10);
 
$graph->title->Set('Forbruk Engangsartikler - Kopipapir');
