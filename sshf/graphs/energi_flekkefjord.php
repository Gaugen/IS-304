...
if ($stmt = $mysqli->prepare("SELECT energi_flekkefjord, energi_sshf_avrg, energi_normalforbruk 
                                      FROM graphs LIMIT 1")){
            $stmt->execute();  
            $stmt->store_result();
			$stmt->bind_result($flekkefjord, $averageSSHF, $average);
			$stmt->fetch();
									  }
									 
$areal = $_GET['n'];
$forbruk = $areal * $flekkefjord;
